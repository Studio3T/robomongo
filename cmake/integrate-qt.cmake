...
MACRO(DEPLOY_QT_PLUGIN_HELPER _path PATH_WHERE_SEARCH LIB_DIST TYPE)
    ...
    IF(${_type} STREQUAL "DEBUG")# try to find debug version of library        
        ...
        FIND_LIB(${PATH_WHERE_SEARCH}/${dir} ${name_local} PLUGIN_${name})  
    ENDIF()
    IF(NOT PLUGIN_${name})
        FIND_LIB(${PATH_WHERE_SEARCH}/${dir} ${name} PLUGIN_${name}) 
    ENDIF()

    IF(PLUGIN_${name})
        GET_FILENAME_COMPONENT(LibWithoutSymLink ${PLUGIN_${name}} REALPATH)
        GET_FILENAME_COMPONENT(PLUGIN_NAME ${PLUGIN_${name}} NAME)
        IF(NOT ${PLUGIN_${name}} STREQUAL ${LibWithoutSymLink})
        ...
        ELSE()
            MESSAGE(STATUS "Deployng ${PLUGIN_${name}} plugin for ${TYPE}")
            INSTALL(FILES ${PLUGIN_${name}} DESTINATION "${LIB_DIST}/${dir}" CONFIGURATIONS ${TYPE} COMPONENT Runtime)
        ENDIF()
