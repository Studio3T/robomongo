...
MACRO(DEPLOY_QT_PLUGIN_HELPER _path LIB_DIST TYPE)
    ...
    #message(${PLUGIN})
    if(EXISTS ${PLUGIN})
        message(STATUS "Deployng ${_path} plugin for ${TYPE}")
        IF(${TYPE} STREQUAL "DEBUG")
            INSTALL(FILES ${PLUGIN} DESTINATION "${LIB_DIST}/plugins/${_dir}" CONFIGURATIONS ${TYPE} COMPONENT Runtime)
        ...
        ELSE()
            INSTALL(FILES ${PLUGIN} DESTINATION "${LIB_DIST}/plugins/${_dir}" CONFIGURATIONS ${TYPE} COMPONENT Runtime)
            #STRING(REPLACE "${QT_IMAGEFORMATS_PLUGINS_DIR}" "${CMAKE_INSTALL_PREFIX}/${PROJECT_NAME}.app/Contents/MacOS/imageformats" 
            #    QT_IMAGEFORMATS_PLUGIN_LOCAL ${QT_${PLUGIN}_PLUGIN_RELEASE}
            #    )
            #LIST(APPEND BUNDLE_LIBRARIES_MOVE ${QT_IMAGEFORMATS_PLUGIN_LOCAL})
        ENDIF()
    else()
...
ELSE()
    ...
    #create symlinks
        FOREACH(dllsToCopy ${DLIBS_TO_COPY})		
            ...
            INSTALL(FILES ${dllsToCopy} DESTINATION ${LIB_DIST} RENAME ${lnname})    
...
IF(APPLE OR WIN32)
    ...
    FOREACH(miscLib ${MISC_LIBS})
        ...
        INSTALL(FILES ${miscLib_release} DESTINATION ${LIB_DIST})
        ENDIF()
...
ELSEIF(UNIX)
    ...
    FOREACH(miscLib ${MISC_LIBS})
        ...
        IF(EXISTS "${LibWithoutSymLink}")
            INSTALL(FILES ${LibWithoutSymLink} DESTINATION ${LIB_DIST} RENAME ${miscLib})	
        ENDIF()
    ENDFOREACH(miscLib ${MISC_LIBS})
ENDIF()	
#deploy_qt_plugins(${QT_PLUGINS}) http://habrahabr.ru/post/132313/
